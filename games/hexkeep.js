(function(global){
 ...
 // soldiers are unchanged; the shipyard's heavier table now actually lands.
 function crewDamage(b,unit){const v=b.buildings[unit.site];return v?TYPES[v.type].damage[v.level-1]:0;}
 function crewRoute(g){return g&&g.route||'land';}
 ...
 function range(v){return TYPES[v.type].range+(v.level>=2&&['guard','shipyard','mage'].includes(v.type)?1:0);}
 // A garrison tower stands at the station its plot declares on its own route:
 // `rally` is a step on the road, `lane` a step on the water.
 function stationStep(M,index,type){const site=M.sites[index],crew=CREW[type];if(!site||!crew)return null;
  const step=crew.route==='water'?site.lane:site.rally,route=M.routes[crew.route];
  return route&&route[step]!==undefined?step:null;}
 // A plot has to be dry land and has to see a route — the road, or the water
 // lane for the coastal batteries.
 function placement(b,index,type){const M=mapOf(b),site=M.sites[index];
  if(!site||!TYPES[type]||M.water&&M.water.has(site.tile))return false;
  // A garrison needs a station on its own route — an island has no road to
  // hold, a shore plot away from the water has no lane to patrol. A shooting
  // tower only needs some route inside its range, whichever one it is.
  if(CREW[type])return stationStep(M,index,type)!==null&&M.routes[CREW[type].route].some(id=>M.distance(id,site.tile)<=TYPES[type].range);
  return Object.keys(M.routes).some(key=>M.routes[key].some(id=>M.distance(id,site.tile)<=TYPES[type].range));}
 function available(b,index,type){const t=TYPES[type];return !!t&&placement(b,index,type)&&!b.buildings[index]&&b.supplies>=t.cost;}
 function newCrew(b,index){const v=b.buildings[index],crew=CREW[v.type];b.guards.push({id:++b.serial,site:index,slot:b.guards.filter(g=>g.site===index).length,enemyId:null,step:v.rally,hp:crew.hp,maxHp:crew.hp,respawn:0,route:crew.route});}
 function build(b,index,type){if(!available(b,index,type))return false;const M=mapOf(b);b.supplies-=TYPES[type].cost;
  b.buildings.splice(index,1,{type,level:1,rally:CREW[type]?stationStep(M,index,type):M.sites[index].rally});
  const crew=CREW[type];if(crew)for(let i=0,n=crew.size(1);i<n;i++)newCrew(b,index);return true;}
 function upgradeCost(v){return v.level===1?TYPES[v.type].upgrade:TYPES[v.type].master;}
 function upgrade(b,index){const v=b.buildings[index];if(!v||v.level>=3||b.supplies<upgradeCost(v))return false;b.supplies-=upgradeCost(v);v.level++;if(CREW[v.type])newCrew(b,index);return true;}
 function rally(b,index,step){const M=mapOf(b),v=b.buildings[index],crew=crewOf(v);if(!crew)return false;
  const route=M.routes[crew.route];if(!route||route[step]===undefined||M.distance(M.sites[index].tile,route[step])>range(v))return false;
  v.rally=step;b.guards.filter(g=>g.site===index).forEach(g=>g.step=step);return true;}
 ...
 function resolveTurn(input,deferHits=false){
  ...
  b.enemies.forEach(e=>{const g=b.guards.find(g=>g.id===e.guardId&&g.hp>0&&g.step===e.step&&crewRoute(g)===routeOf(e));if(g&&!e.boss&&!occupied.has(g.id)){occupied.add(g.id);g.enemyId=e.id;}else{e.guardId=null;e.duelTicks=0;}});
  b.enemies.forEach(e=>{
   const pace=e.speed||1;
   e.previous=e.step;
   let defender=b.guards.find(g=>g.id===e.guardId);
   // A fast enemy still runs into any defender standing inside its stride, so
   // extra speed slips past towers rather than past the line. Only a crew on
   // the same route can stand in the way at all.
   if(!defender)defender=b.guards.find(g=>g.hp>0&&crewRoute(g)===routeOf(e)&&!occupied.has(g.id)&&g.step>=e.step&&g.step<=e.step+pace);
   if(!defender){e.step+=pace;return;}
   occupied.add(defender.id);
   if(e.boss){e.step+=pace;b.impacts.push({kind:'melee',enemyId:e.id,guardId:defender.id,damage:crewDamage(b,defender),at:440,done:false});return;}
   ...
   if(e.duelTicks%2===1)b.impacts.push({kind:'melee',enemyId:e.id,guardId:defender.id,damage:crewDamage(b,defender),at:440,done:false});
  });
  // A garrison tower fights through its crew and never also shoots.
  b.buildings.forEach((v,i)=>{if(!v||CREW[v.type])return;if(b.turn%TYPES[v.type].every)return;
 ...
 function createHexkeepComponent(Base){return Vue.component('hexkeep',Vue.extend({
  extends:Base,
  data(){return {newWords:[],newWordIndex:0,menuOpen:false,menuPosition:{},battle:freshBattle(),phase:'opening',resumePhase:'planning',question:null,options:[],selectedSite:0,selectedType:'guard',feedback:'',wrongIndex:-1,stopped:false,run:0,turnTimer:null,answerTimer:null,motion:[],reduced:false,hidden:false,types:TYPES,crews:CREW,kinds:KINDS,levels:LEVELS,cleared:[],saves:{},art:global.HEXKEEP_ART,atlas:ATLAS,atlasPick:null,atlasFresh:null,debugAvailable:debugRequested(),debugMode:false,debugPoints:DEBUG_POINTS};},
  computed:{kit(){return themeKit(this.theme);},map(){return mapOf(this.battle);},level(){return levelOf(this.battle);},learning(){return this.newWords.length>0||['ready','retry','resolved'].includes(this.phase);},canPlan(){return !this.newWords.length&&!this.stopped&&['planning','combat','opening'].includes(this.phase);},selected(){return this.battle.buildings[this.selectedSite];},rangeTiles(){if(!this.menuOpen)return [];const M=this.map,v=this.selected||{type:this.selectedType,level:1};return M.tiles.filter(t=>M.distance(t.id,M.sites[this.selectedSite].tile)<=range(v));},raidSize(){return targetCount(this.battle);},needed(){return 0;},rallySteps(){const M=this.map,crew=this.selected&&CREW[this.selected.type];if(!crew)return [];
    return (M.routes[crew.route]||[]).map((tile,step)=>({tile,step})).filter(p=>M.distance(M.sites[this.selectedSite].tile,p.tile)<=this.selected.level);},
   // An island has no road to garrison and a dry region has no lane to patrol,
   // so a plot is only ever offered the towers it can really hold.
   buildableTypes(){const out={};for(const key of Object.keys(TYPES))if(placement(this.battle,this.selectedSite,key))out[key]=TYPES[key];return out;},sceneSummary(){return 'כפר עם שביל מפותל. '+this.battle.enemies.length+' אויבים בדרך. לכפר נותרו '+this.battle.health+' לבבות.';},
  ...
  template:`<main class="hk-game" :style="kit" :class="{'hk-reduced':reduced,'hk-hidden':hidden,'hk-question-open':learning}" @keydown="onKey" dir="rtl">
   ...
   <div class="hk-layout"><section class="hk-world" aria-label="מפת הכפר">
    ...
    <section v-if="menuOpen&&canPlan" ref="buildMenu" tabindex="-1" class="hk-build-menu" :style="menuPosition" role="dialog" :aria-label="selected?'שדרוג מגדל':'קניית מגדל'" @keydown.esc.stop="closeBuildMenu">
     ...
     <div v-if="!selected" class="hk-blueprints"><button v-for="(type,key) in buildableTypes" :key="key" :disabled="!canBuyType(key)" @focus="selectedType=key" @mouseenter="selectedType=key" @click="buyType(key)"><img :src="'assets/hexkeep/'+key+'.png'" alt=""><span><b>{{type.name}}</b><small>{{type.subtitle}}</small><i class="hk-power">{{powerLabel(key,1)}}</i><em>{{type.cost}} נקודות</em></span></button></div>
     <template v-else><img class="hk-menu-tower" :src="'assets/hexkeep/'+towerArt(selected)+'.png'" alt=""><p class="hk-detail">{{types[selected.type].detail}}</p><p class="hk-power hk-power-line">{{powerLabel(selected.type,selected.level)}}<span v-if="selected.level<3"> ← אחרי שדרוג: {{powerLabel(selected.type,selected.level+1)}}</span></p><button class="hk-primary hk-upgrade" :disabled="selected.level===3||!canUpgrade()" @click="upgradeSelected">{{selected.level===3?'משודרג עד הסוף':'שדרוג · '+upgradePrice()+' נקודות'}}</button><p v-if="types[selected.type]&&crews[selected.type]" class="hk-detail">{{guardStatus()}}</p></template>
    </section>
   </section>
  ...
  methods:{
   ...
   loadSlot(id){const slot=this.saves[id],map=levelOf({level:id}).map;
    ...
    this.battle=slot.battle;this.battle.level=id;this.battle.buildings.forEach((v,i)=>{const crew=crewOf(v);if(crew){const units=this.battle.guards.filter(g=>g.site===i);units.forEach((g,slot)=>{g.slot=slot;g.route=g.route||crew.route;});while(this.battle.guards.filter(g=>g.site===i).length<crew.size(v.level))newCrew(this.battle,i);}});
   ...
   sailorStyle(g){const M=this.map,P=this.routeTiles(g),meta=this.walkMeta({kind:'patrol'}),step=Math.max(1,Math.min(g.step,P.length-1)),
    ...
    return {backgroundPosition:(frame/(meta.frames-1)*100)+'% '+(row/(meta.directions-1)*100)+'%'};},
   ...
   powerLabel(type,level){const t=TYPES[type],crew=CREW[type];
    return crew?(type==='shipyard'?'⚓ ':'🛡 ')+crew.size(level)+' '+(type==='shipyard'?'סירות':'חיילים'):'⚔ עוצמה '+t.damage[level-1]*6/t.every;},
   ...
   guardStatus(){const crew=CREW[this.selected&&this.selected.type],unit=crew?crew.unit:'שומר';
    return this.battle.guards.filter(g=>g.site===this.selectedSite).map(g=>g.hp>0?unit+': '+g.hp+'/'+g.maxHp+' חיים':unit+' בתיקון').join(' · ');},
 ...
 global.HEXKEEP={campaign:CAMPAIGN,levels:LEVELS,kinds:KINDS,levelOf,mapOf,nextLevel,canStartRaid,rewardAnswer,prepareRaid,range,upgradeCost,types:TYPES,crew:CREW,freshBattle,placement,available,build,upgrade,resolveTurn,applyImpacts,themeKit,targetCount,rally,routeOf,pathOf,armorOf,stationStep};global.createHexkeepComponent=createHexkeepComponent;
