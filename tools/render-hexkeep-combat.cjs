...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 const variants=[{model:'Skeleton_Minion',clip:'1H_Melee_Attack_Slice_Horizontal',file:'enemy-fight'},{model:'Knight',clip:'1H_Melee_Attack_Slice_Horizontal',file:'guard-fight'}];for(const variant of variants){const result=await page.evaluate(async variant=>{
  const T=THREE,asset=await new T.GLTFLoader().loadAsync('/assets/models/duel/'+variant.model+'.glb'),model=asset.scene;
  const clip=asset.animations.find(c=>c.name===variant.clip);if(!clip)throw Error('Attack clip missing');
  model.scale.setScalar(variant.model==='Knight'?.36:.38);const pivot=new T.Group();pivot.add(model);const scene=new T.Scene();scene.add(pivot);
 ...
 },variant);const out=path.resolve('assets/hexkeep');fs.writeFileSync(path.join(out,variant.file+'.png'),Buffer.from(result.png,'base64'));console.log('Baked '+variant.file+' attack atlas');}
