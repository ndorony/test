...
g.saveCampaign();const regular=JSON.stringify(g.battle);g.toggleDebug();check(g.debugMode&&g.battle.supplies===100&&g.battle.earned===0,'debug starts with 100 currency and no invented answers');g.battle.supplies=7;g.saveCampaign();g.toggleDebug();check(!g.debugMode&&JSON.stringify(g.battle)===regular,'debug purchases never overwrite regular village');g.toggleDebug();check(g.battle.supplies===100,'each fresh debug session starts with 100');g.toggleDebug();
