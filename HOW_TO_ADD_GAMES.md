...
| `crystal_arena` | Crystal Arena | Camera game: four answer crystals in the four screen corners. The player holds a saturated coloured object (glove, ball, sticky note) and rests it in the corner of their answer; the game calibrates to that hue once, then tracks the *object* — position, velocity and size carry its identity, so it can be followed through shadow and changing light while the colour envelope learns its other looks. No model or library. Click/touch/keyboard always work; the camera is optional |
