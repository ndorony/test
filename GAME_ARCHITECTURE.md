...
| `scribble_dungeon` | `ScribbleDungeonComponent` (`games/scribble-dungeon.js`) | `listName, questionIndex, resultIndex, setItems, title` | מבוך הקלף — מפת מבוך שמצוירת על דף מחברת משובץ תוך כדי משחק (קנבס דו-ממדי, אריחי Kenney Scribble Dungeons ב-CC0). מחולל חדרים בגדלים משתנים (תאים, אולמות, מסדרונות ארוכים, מערות) שנצמדים זה לזה כמו תוכנית קומה אמיתית. כל תשובה יושבת על דלת; נכונה = הדלת נפתחת והדמות צועדת לחדר הבא שנרשם על הדף תוך כדי הליכה, שגויה = הדלת נחסמת ב-X והחדר שמאחוריה נשאר על המפה מסומן. חדרים שכבר נחקרו לא נמחקים — המפה גדלה ונשארת גלויה. כל הספרייטים נצבעים מחדש בזמן ריצה (`inked`) לפי ערכת הנושא |
| `crystal_arena` | `CrystalArenaComponent` (`games/crystal-arena.js`) | `listName, questionIndex, resultIndex, setItems, title` | זירת הגבישים — משחק מצלמה. ארבעה גבישי תשובה בארבע פינות המסך מעל תמונת המצלמה (משוקפת), והשאלה בליבת הזירה. השחקן מחזיק **חפץ בצבע רווי** — כפפה, כדור, פתק צהוב — ומניח אותו בפינה של התשובה הנכונה. מסך כיול חד-פעמי קורא את הגוון הדומיננטי מתוך מסגרת מיקוד (היסטוגרמה מעגלית של 36 תאים, ננעל אחרי ~0.4 שניות יציבות) ושומר אותו ב-`crystal_arena_colour_profile`; אחר כך המעקב הוא התאמת גוון ב-HSV, כלומר הוא יודע *מה* הוא מחפש — צל, כתף או ריהוט פשוט לא מתאימים. הגביש נטען כל עוד החפץ נח בפינה (dwell) ורק אז מנפץ, והוא מתחמש מחדש רק כשהחפץ יוצא. שתי גישות קודמות (הפרשי פריימים, חיסור רקע) נכשלו כי לא היה להן מושג מהי יד. לחיצה/מגע/מקלדת (1-4 וחיצים) עובדים תמיד; המצלמה היא שדרוג בלבד. שום פריים לא נשמר ולא נשלח |
